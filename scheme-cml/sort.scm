...
(define (list-merge-sort elt< lis)
  ...
  (if (pair? lis)				; Don't sort an empty list.
      (mlet (((r lr tail)  (getrun lis))	; Pick off an initial run,
	     ;; [TRC 20090914: Use (LENGTH LIS), not #o100000000.]
	     ((infinity)   (length lis))	; then grow it up maximally.
...
(define (list-merge-sort! elt< lis)
  ...
  (if (pair? lis)
      (mlet (((lr endr rest)  (getrun lis))	; Pick off an initial run.
	     ;; [TRC 20090914: Use (LENGTH LIST), not #o100000000.]
	     ((infinity)      (length lis))	; Then grow it up maximally.
...
;;; [TRC 20090914: Define the algorithm-generic wrappers.]
